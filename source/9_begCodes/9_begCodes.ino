...
typedef struct Codes
{
  ...
  char currentWeekCode[CODELENGTH +1];  //so of type char*

  char nextWeekCode[CODELENGTH +1];

  int currentDeliverySlotNumber;
  char currentDeliveryCode[CODELENGTH +1];
  
  char nextDeliveryCode[CODELENGTH +1];
}Codes;
...
void weekChange()
{
  ...
  copyCodes(codes.currentWeekCode, codes.nextWeekCode);
  setRandCodes(codes.nextWeekCode);
}
...
void codesSetup()
{
  setRandCodes(codes.nextWeekCode);
  weekChange();
  setRandCodes(codes.currentDeliveryCode);
  setRandCodes(codes.nextDeliveryCode);
}
...
void codesLoop()
{
  ...
  Serial.print(codes.currentWeekCode);
  Serial.print(" next week code: ");
  Serial.println(codes.nextWeekCode);
