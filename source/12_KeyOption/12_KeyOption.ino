...
void keyLoop(void)
{
  ...
  if ( (voltExpected * 0.9 < voltage) && (voltage < 1.1 * voltExpected) )
    keyStatus = GOODKEY;
   else if (voltage < 1023 / 20) //meaning very low value: 5% of full scale
          keyStatus = NOKEY;
        else
          keyStatus = FAKEKEY;
  ...
  Serial.println();
  Serial.print("Voltage of the A5: ");
...
void processBuff(void)
{
  if (nbBuff > 0 && nbBuff < BUFFKEYSIZE && tabBuff[nbBuff-1] == '#')
  {
    ...
    if (nbBuff==6 && tabBuff[0] == '*') //*2134# for 21h34 -> administrator
    {
    ...
    if (nbBuff==10 && tabBuff[0] == '*') //*19032022# for 19th March 2022 -> administrator
    {
      int d;
      int m;
      int y;
    ...
    if (nbBuff==4 && tabBuff[0] == '*') // *06# for 6° and *95# for -5° //for maintenance only -> administrator
    {
      int sign;
      int value;
    ...
    if (nbBuff==7) //213532# for 213532
    {
      ...
      if (strncmp(tabBuff, codes.currentWeekCodeS, 6) == 0 || 
